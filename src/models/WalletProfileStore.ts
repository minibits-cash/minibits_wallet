...
export const WalletProfileStoreModel = types
    ...
    .actions(self => ({  
        create: flow(function* create(walletId: string, seedHash: string) {
       
            let profileRecord: WalletProfileRecord            

            log.trace('[create]', {seedHash, walletId})
        ...
        recoverAddress: flow(function* recoverAddress(seedHash: string) {
            ...
            log.info('[recoverAddress] ', 'Wallet address recovered in WalletProfileStore', {self})
            return self
        }),
